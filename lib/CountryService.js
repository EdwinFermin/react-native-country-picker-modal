...
export const getCountriesAsync = async (flagType, translation = 'common', region, subregion, countryCodes, excludeCountries, preferredCountries, withAlphaFilter) => {
    ...
    if (preferredCountries && !withAlphaFilter) {
        ...
        const newCountryCodeList = sortedCountryCodeList.filter(code => !preferredCountries.includes(code));

        const preferredCountriesMap = preferredCountries.filter(isCountryPresent(countriesRaw))
            .map((cca2) => ({
            cca2,
            ...{
                ...countriesRaw[cca2],
        ...
        return [...preferredCountriesMap, ...countries];
    }
    else {
